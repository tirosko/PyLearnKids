...
def game_over():
    while True:
        mouse1 = pygame.mouse.get_pos()  # Update mouse position each loop
        for ev in pygame.event.get():
            ...
            if ev.type == pygame.MOUSEBUTTONDOWN:
                ...
                if width - 180 < mouse1[0] < width - 100 and height - 100 < mouse1[1] < height - 80:
                    game(lead_y, lead_x, speed, count)  # Correct order
...
def game(
    ...
    global e_p, e1_p, width, height  # Declare globals if you modify them
    while True:
        for ev in pygame.event.get():
            if ev.type == pygame.QUIT:
                pygame.quit()
